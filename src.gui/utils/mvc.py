...
def load_methods(cls, module, reloading):
    def fgetter(funcname):
        ...
        fgetterreal._fname = funcname
        return fgetterreal

    for funcname in dir(module):
        if funcname.startswith('_') and funcname != '__init__':
            continue
...
def _subscribe(self, pubmethod=None, pubname=None):
    ...
    if pubmethod is not None:
        pmname = pubmethod._fname  # methods have already been fgettereal'ed
        pmethod = getattr(self.__class__, pmname)
        methods = [[pmname, pmethod]]
    else:
        methods = inspect.getmembers(self.__class__, inspect.ismethod)
    for mname, method in methods:
        pubsubtopic = pubname or getattr(method, '_pubrecv', None)
        if pubsubtopic:
            self._subs[mname] = method
            # subsgetter = pubmethod or sgetter(mname)
            ...
            if (not topicmgr.getTopic(pubsubtopic, True) or
                ...
                pub.subscribe(
                    subsgetter.__get__(self, self.__class__),
                    pubsubtopic)
...
class PubBindSlave(object):
    def __init__(self, name):
        self._pubrecv = name

    def __getattribute__(self, name):
        _pubrecv = object.__getattribute__(self, '_pubrecv')
        if name == '_pubrecv':
            return _pubrecv
        self._pubrecv += '.' + name
        return self
    ...
    def __call__(self, func):
        func._pubrecv = self._pubrecv
        return func


class MetaPubRecv(type):
    def __getattribute__(cls, name):
        return PubBindSlave(name)


class PubRecv(object):
    __metaclass__ = MetaPubRecv
