...
class MetaWidget(type):
    def __getattribute__(cls, name):
        ...
        seen = type.__getattribute__(cls, '_seen')
        if seen:
            return type.__getattribute__(cls, name)

        def decorator(*args, **kwargs):
            cls._seen = True
            cls.__bases__[0]._seen = True
            ...
            return wrapper(name, *args, **kwargs)
        return decorator
...
class BindingWidget(object):
    ...
    def __init__(self, name, **kwargs):
        ...
        self.findwidget()
        pubmethod = kwargs.pop('pubmethod', None)
        self.createvars(**kwargs)
        self.dobindings(pubmethod=pubmethod)
    ...
    def createvars(self, **kwargs):
        ...
        for binding in self.bindings:
            ...
            for key in ['config']:  # loop in case other keys would be needed
                if key in kwargs:
                    bindkwargs[key] = kwargs[key]
    ...
    def install(self):
        for i in itertools.count(1):
            ...
            owner = frame.f_locals.get('self', None)
            if owner is None:
                continue
    ...
    def dobindings(self, pubmethod=None):
        ...
        for methodname, method in members:
            if hasattr(method, AutoBind.attrname):
                ...
                if pubmethod is not None:
                    mvc._subscribe(self.owner, pubmethod, pubname)
...
class BindingListBox(BindingWidget):
    wprefix = 'listBox'
    ...
    @AutoBind.EVT_LISTBOX
    def OnListBox(self, event):
        assert doout(event)
        event.Skip()
        selection = event.GetInt()
        self._set('selection', selection)
    ...
    def OnStringSelectionChange(self, value):
        ...
        if not self.widget.GetCount():  # no items
            return value

        if self.widget.SetStringSelection(value):
            # The variable must contain the reality and not what was sent,
            # because the operation may fail if the "value" is not in the list
            # of items (use _set to avoid an infinite loop)
            value = self.widget.GetStringSelection()
            self._set('stringselection', value)
